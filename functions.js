...
function addWheel(userid, url, title, wheelid) {
	...
	xmlhttp.open("GET", hostName + "/fn-add.php?u=" + userid + "&t=" + enctitle
			+ "&url=" + encurl + "&wheelid=" + wheelid, true);
	xmlhttp.send();
}
...
function updateWheel(userid, url, title, wheelid) {
	...
	var enctitle = encodeURIComponent(title);
	xmlhttp.open("GET", hostName + "/fn-add.php?u=" + userid + "&t=" + enctitle
			+ "&url=" + encurl + "&mod=1&wheelid=" + wheelid, true);
	xmlhttp.send();
}
...
function deleteWheel(userid, id) {
	...
	xmlhttp.open("GET", hostName + "/fn-delete.php?u=" + userid + "&id=" + id,
			true);
	xmlhttp.send();
}